//HintName: DarkLink.RoslynHelpers.AttributeGenerator.Marker.g.cs
// <auto-generated />
#nullable enable
...
{
    partial record Marker
    {
        public const string ATTRIBUTE_NAME = "DarkLink.RoslynHelpers.AttributeGenerator.Marker";

        public static void AddTo(IncrementalGeneratorPostInitializationContext context)
        {
            const string hintName = "DarkLink.RoslynHelpers.AttributeGenerator.Marker.g.cs";
            const string code = @"using System;

namespace DarkLink.RoslynHelpers.AttributeGenerator
{
    [AttributeUsage((AttributeTargets)32767, AllowMultiple = false, Inherited = true)]
    internal class Marker : Attribute
    {
        public Marker() { }
    }
}
";
        ...
        public static Marker From(AttributeData data)
        {
        ...
        public static IncrementalValuesProvider<T> Find<T>(
            ...
            Func<GeneratorAttributeSyntaxContext, IReadOnlyList<(AttributeData AttributeData, Marker ParsedData)>, CancellationToken, T> transform)
            => syntaxValueProvider.ForAttributeWithMetadataName(
                ATTRIBUTE_NAME,
                predicate,
                (context, cancellationToken) =>
                {
                    var attributes = (IReadOnlyList<(AttributeData, Marker)>) context.Attributes
                        .Select(data =>
                        {
        ...
        public static IncrementalValuesProvider<T> Find<T>(
            SyntaxValueProvider syntaxValueProvider,
            Func<SyntaxNode, CancellationToken, bool> predicate,
            Func<GeneratorAttributeSyntaxContext, IReadOnlyList<Marker>, CancellationToken, T> transform)
            => Find(
                syntaxValueProvider,
                predicate,
